...
export class PlinkoComponent implements OnInit {
  ...
  lastMultiplierX: number =
    worldConfig.width / 2 -
    (pinsConfig.pinGap / 2) * this.lines -
    pinsConfig.pinGap;
  ...
  constructor(
    ...
    this.httpService
      .postRequest('basic/Specificgame', { name: 'plinko' }, this.token)
  ...
  async addBall() {
    ...
    const ballColor =
      this.ballValue <= 0 ? colorsConfig.text : colorsConfig.purple;
  ...
  allUser_history() {
    this.httpService
      .postRequest('plinko/getAllBetHistory', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  getDetails(id: any) {
    ...
    this.httpService
      .postRequest('plinko/getDetails', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  userFav(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('plinko/userFav', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  userLike(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('plinko/userLiked', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
