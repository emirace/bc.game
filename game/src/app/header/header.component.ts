...
export class HeaderComponent implements OnInit {
      ...
      if (pathway == 'activate_account') {
        this.actRoute.params.subscribe((params) => {
          ...
          if (
            ...
            this.httpService
              .postUrl('basic/activateEmail', { token: this.loginToken })
              .subscribe((res: any) => {
                if (res.success == 1) {
        ...
        this.actRoute.params.subscribe((params) => {
          ...
          if (
            ...
            this.resetToken == 'undefined' ||
            this.resetToken == ' ' ||
            this.resetVerify == null ||
            this.resetVerify == undefined ||
            this.resetVerify == 'undefined' ||
            this.resetVerify == ''
          ) {
  ...
  checkname(user: any) {
    if (user.length >= 6) {
      if (/^[a-zA-Z0-9_]+$/.test(user)) {
        ...
        this.httpService
          .postUrl('basic/findUsername', data)
          .subscribe((res: any) => {
            if (res.success == 0) {
              this.usrname = false;
              this.alert.error(res.msg, '', { timeOut: 2000 });
  ...
  executeAction(type: any, form: NgForm) {
    if (type == 'login') {
      ...
      if (!this.usrmail) {
        this.alert.error('Email already exists', '', { timeOut: 2000 });
        return;
      }
      if (this.lower && this.upper && this.len && this.num && this.spec) {
      } else {
        this.alert.error('password not valid', '', { timeOut: 2000 });
        return;
      }
      ...
      if (!this.checkpwd) {
        this.alert.error('Password does not match !', '', { timeOut: 2000 });
        return;
      }
  ...
  loginProcess(form: NgForm) {
    ...
    this.httpService.postUrl('basic/login', data).subscribe((res: any) => {
      if (res.success == 1) {
        ...
        this.alert.success('Please login with Authentication code', '', {
          timeOut: 2000,
        });
        this.openPopup();
      } else {
  ...
  forgotPass(form: NgForm) {
    var data = form.value;
    this.httpService
      .postUrl('basic/forgotPassword', data)
      .subscribe((res: any) => {
        if (res.success == 1) {
          this.alert.success('Reset-password link sent to your email', '', {
            timeOut: 2000,
          });
          form.resetForm();
        } else {
  ...
  resetPassword(form: NgForm) {
    ...
    this.httpService
      .postUrl('basic/reset_password', data)
      .subscribe((res: any) => {
        if (res.success == 1) {
          this.alert.success(res.msg, '', { timeOut: 2000 });
          this.route.navigate(['/']);
