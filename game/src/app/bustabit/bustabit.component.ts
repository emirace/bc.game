...
export class BustabitComponent implements OnInit {
  ...
  bettingList: any = [];
  allBethistory: any = [];
  payoutAllHistory: any = [];
  showMoreHis: any = false;
  seedset: any = true;
  showMore: any = false;
  ActiveBetId: any;
  BetLimit: any = false;
  betResultsDetails: any = { betList: [] };
  Bethistory: any = [];
  fullHis: any = [];
  allbet: any = [];
  ...
  constructor(
    ...
    this.httpService
      .postRequest('basic/Specificgame', { name: 'bustabit' }, this.token)
  ...
  async bettingInitiate(data: any) {
    ...
    if (resData.success == 1) {
      ...
      var data1 = {
        ...
        clientSeed: resData.clientSeed,
        serverSeed: resData.serverSeed,
        nonce: resData.nonce,
        result: this.winning_result,
      };

      var resData1: any = await this.httpService.postRequestUpdated(
        'bustabit/saveBetHistory',
        data1,
        this.token
      );
  ...
  showAllHis() {
    this.httpService
      .postRequest('bustabit/getAllBetHistory', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  showuserBet() {
    this.httpService
      .getRequest('bustabit/getUserBetHistory', this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  Details(id: any) {
    var obj = { _id: id };
    this.httpService
      .postRequest('bustabit/getDetails', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  userFav(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('bustabit/userFav', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  userLike(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('bustabit/userLiked', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  getgraph() {
    let data = { game: 'limbo' };
    this.httpService
      .postRequest('bustabit/getgraphed', data, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
          if (resData.msg == 0) {
  ...
  getExacttime() {
    let data = { game: 'limbo' };
    this.httpService
      .postRequest('bustabit/getExacttime', data, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
          this.getgraph();
        }
