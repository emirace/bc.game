...
export class InnerheaderComponent implements OnInit {
    ...
    if (actRoute.snapshot.url[0] !== undefined) {
      if (actRoute.snapshot.url[0].path == 'activate_withdraw') {
        this.actRoute.params.subscribe((params) => {
          ...
          if (
            this.withToken == null ||
            this.withToken == undefined ||
            this.withToken == 'undefined' ||
            this.withToken == '' ||
            this.withVerify == null ||
            this.withVerify == undefined ||
            this.withVerify == 'undefined' ||
            this.withVerify == ''
          ) {
            ...
            this.httpService
              .postUrl('withdraw/activatewithdraw', data)
              .subscribe((res: any) => {
                if (res.success == 1) {
  ...
  kycDet() {
    this.httpService
      .getRequest('basic/Getstatus', this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
          if (
            resData.userData.tfa_status == 1 ||
            resData.userData.kyc_status == 3
          ) {
            this.checkBtn = true;
          } else {
            this.checkBtn = false;
  ...
  userwallet() {
    this.httpService
      .getRequest('basic/getvaluBal', this.token)
      .subscribe((resData: any) => {
        if (resData) {
  ...
  currselect(curr: any) {
    this.httpService
      .postRequest('basic/getCurrencyInfo', { currency: curr }, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  depositData(curr: any) {
    if (curr != 'APE') {
      this.httpService
        .postRequest('withdraw/getAddress', { currency: curr }, this.token)
        .subscribe((resData: any) => {
          if (resData.success == 1) {
  ...
  getUserData() {
    this.httpService
      .getRequest('basic/getUserdetals', this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
          ...
          this.referalLink =
            host + '/referrals?referral=' + resData.user.referr_id;
          var iframe: any = document.getElementById('myFrame');
          if (iframe !== null) {
            iframe.src = iframe.src + '?username=' + resData.user.username;
          }
          this.userNamePass();
        } else {
          this.alert.error('somthing wents wrong !', '', { timeOut: 2000 });
  ...
  shoeCurr() {
    this.httpService
      .postRequest('basic/getWalletAmt', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
      ...
      if (type == 'withdraw') {
        if (data.address == undefined || data.withdrawAmt == 0) {
          ...
          var obj = {
            ...
            amount: this.with_amount,
            transfer_amount: this.totalwith,
            currency: this.WithCurr,
            currency_type: this.type,
            address_info: this.withaddress,
            fee_amt: this.fee,
          };
  ...
  WithProcess(data: any) {
    this.httpService
      .postRequest('withdraw/withdrawprocess', data, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  openPopup() {
    ...
    this.httpService
      .getRequest('basic/getUsrinfo', this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  updateUsername() {
    ...
    this.httpService
      .postRequest('basic/updateUsername', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  chooseImg(id: any) {
    var obj = { img: id };
    this.httpService
      .postRequest('basic/profileImg', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  statDet(id: any) {
    ...
    this.httpService
      .getRequest('basic/getgameInfo', this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  notifydata() {
    this.httpService
      .getRequest('basic/getNotifyData', this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
