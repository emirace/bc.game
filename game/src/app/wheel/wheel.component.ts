...
export class WheelComponent implements OnInit {
  ...
  value3: number = 10;
  options3: Options = { floor: 10, ceil: 50, step: 10 };
  ...
  colors: any;
  risk_value: any = 'Low';
  segment_value: any = 10;
  seed1: any = [];
  values: any;
  winAmount: any;
  selectedValue: any = 'Low';
  wheelResult: any = [];
  chance: any;
  ...
  constructor(
    ...
    this.httpService
      .postRequest('basic/Specificgame', { name: 'wheel' }, this.token)
  ...
  change(value: any, option: any) {
    ...
    if (
      this.risk_value == 'High' &&
      (this.segment_value == 20 ||
        this.segment_value == 30 ||
        this.segment_value == 40 ||
        this.segment_value == 50)
    ) {
      this.items = this.seed.map((value) => ({
  ...
  over(data: any, clr: any) {
    ...
    for (const colorObj of uniqueColors) {
      const count = (this.items as { fillStyle: string }[]).filter(
        (item) => item.fillStyle === colorObj
      ).length;
          ...
          if (results.success == 1) {
            ...
            for (var j = 0; j <= this.numberofbets; j++) {
              ...
              if (
                ...
                this.wheel.reset();
                results = await this.httpService.postRequestUpdated(
                  'wheel/getGameResult',
                  data,
                  this.token
                );
  ...
  allUser_history() {
    this.httpService
      .postRequest('wheel/getAllBetHistory', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  getDetails(id: any) {
    ...
    this.httpService
      .postRequest('wheel/getDetails', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  Seedsetting() {
    this.httpService
      .postRequest('wheel/getHash', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  generateSeed() {
    ...
    this.httpService
      .postRequest('wheel/generateHash', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  updateHash() {
    ...
    this.httpService
      .postRequest('wheel/changeHash', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  userFav(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('wheel/userFav', obj, this.token)
      .subscribe((resData: any) => {
  ...
  userLike(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('wheel/userLiked', obj, this.token)
      .subscribe((resData: any) => {
