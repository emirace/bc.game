...
export class CaveofplunderComponent implements OnInit {
  ...
  constructor(
    ...
    this.httpService
      .postRequest('basic/Specificgame', { name: 'caveofplunder' }, this.token)
  ...
  recallData() {
    ...
    if (gameCode !== undefined && gameCode !== null) {
      this.httpService
        .postRequest('cave/recallData', { gameCode: gameCode }, this.token)
        .subscribe((resData: any) => {
          if (resData.success == 1) {
  ...
  async bettingInitiate(data: any) {
    if (!this.isSpinning) {
      ...
      var data1 = {
        ...
        currency: this.currency,
        book_payout: this.book_payout,
        cross_payout: this.cross_payout,
        diamond_payout: this.diamond_payout,
      };
      var resData: any = await this.httpService.postRequestUpdated(
        'cave/saveBetHistory',
        data1,
        this.token
      );
      ...
      if (resData.success == 1) {
        ...
        if (resData.bet_result == 'book') {
          if (resData.book_payout_status == 'reward') {
            //popup
            var toalresult =
              parseFloat(resData.reward_payout) * parseFloat(this.betamount);
            this.winAmt = toalresult;
            this.winBoxMsg();
          }
          let element = this.elementRef.nativeElement.querySelector(
            '#book_' + resData.book_payout * 100
          );
          ...
          if (resData.cross_payout_status == 'spin') {
            ...
            let elements =
              this.elementRef.nativeElement.querySelectorAll('.cross_cls');
            elements.forEach((el) => {
          ...
          let element = this.elementRef.nativeElement.querySelector(
            '#cross_' + resData.cross_payout * 100
          );
          this.renderer.addClass(element, 'wins');
          ...
          let element = this.elementRef.nativeElement.querySelector(
            '#diamond_' + resData.diamond_payout * 100
          );
          this.renderer.addClass(element, 'wins');
        } else if (resData.bet_result == 'skull') {
          if (this.book_payout != 0) {
            let element = this.elementRef.nativeElement.querySelector(
              '#book_' + this.book_payout * 100
            );
            this.renderer.removeClass(element, 'wins');
          }

          if (this.cross_payout != 0) {
            let element1 = this.elementRef.nativeElement.querySelector(
              '#cross_' + this.cross_payout * 100
            );
            this.renderer.removeClass(element1, 'wins');
          }

          if (this.diamond_payout != 0) {
            let element2 = this.elementRef.nativeElement.querySelector(
              '#diamond_' + this.diamond_payout * 100
            );
  ...
  async collect_amount(type: any) {
    if (this.cashpot != 0) {
      ...
      if (resData.success == 1) {
        ...
        this.winBoxMsg();
        this.toastr.success(
          data1.cashpot + ' ' + this.currency + ' credited your wallet'
        );
      }
    }
  }
  ...
  allUser_history() {
    this.httpService
      .postRequest('cave/getAllBetHistory', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  Details(id: any) {
    ...
    this.httpService
      .postRequest('cave/getDetails', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  Seedsetting() {
    this.httpService
      .postRequest('cave/getHash', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  updateHash() {
    ...
    this.httpService
      .postRequest('cave/changeHash', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  generateSeed() {
    ...
    this.httpService
      .postRequest('cave/generateHash', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  userFav(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('cave/userFav', obj, this.token)
      .subscribe((resData: any) => {
  ...
  userLike(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('cave/userLiked', obj, this.token)
      .subscribe((resData: any) => {
