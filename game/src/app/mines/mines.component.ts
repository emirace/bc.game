...
export class MinesComponent implements OnInit {
  ...
  payoutList: any = [1.03, 1.08, 1.13, 1.18, 1.24, 1.3, 1.38];
  ...
  constructor(
    ...
    this.httpService
      .postRequest('basic/Specificgame', { name: 'mines' }, this.token)
  ...
  gameId() {
    ...
    if (checkdata !== null && checkdata !== undefined && checkdata !== '') {
      ...
      this.httpService
        .postRequest('mines/getcurt', { _id: this.betId }, this.token)
        .subscribe((resData: any) => {
          if (resData) {
            this.refreshTiles();
            ...
            this.profit_next = (
              this.next_payout *
              parseFloat(resData.currData.bet_amount) *
              1
            ).toFixed(6);
            this.curr_payout = (
              parseFloat(resData.currData.payout) * 1
            ).toFixed(2);
            this.profit = (
              this.curr_payout *
              parseFloat(resData.currData.bet_amount) *
  ...
  fullResultAppend(result: any) {
    for (var i = 1; i <= 25; i++) {
      ...
      if (
        ...
        element.classList.contains('mines')
      ) {
  ...
  async checkresult(choosenumber: any) {
    if (this.userChoice.indexOf(choosenumber) !== -1) {
      return;
    }
  ...
  showAllHis() {
    this.httpService
      .postRequest('mines/getAllBetHistory', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
    ...
    for (var i = 0; i <= this.numberofbets; i++) {
      if (this.auto_bet_amount <= this.balance) {
        var data = {
          ...
          currency: this.currency,
          user_choice: this.userChoiceTiles,
          mines: this.auto_mines,
        };
        if (
  ...
  Details(id: any) {
    ...
    this.httpService
      .postRequest('mines/getDetails', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
          ...
          for (let i = 0; i < userbet.length; i++) {
            if (userbet[i].bet_result == 'diamond') {
              ...
              jQuery('#' + userbet[i].userchoice + 'res').addClass(
                'selected mines'
              );
            }
          }
  ...
  generateSeed() {
    ...
    this.httpService
      .postRequest('mines/generateHash', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  updateHash() {
    ...
    this.httpService
      .postRequest('mines/changeHash', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  Seedsetting() {
    this.httpService
      .postRequest('mines/getHash', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  userFav(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('mines/userFav', obj, this.token)
      .subscribe((resData: any) => {
  ...
  userLike(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('mines/userLiked', obj, this.token)
      .subscribe((resData: any) => {
