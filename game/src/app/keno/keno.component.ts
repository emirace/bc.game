...
export class KenoComponent implements OnInit {
  ...
  newclientSeed: any;
  activebet: any = false;
  actdisable: any = false;
  autoactdisable: any = false;
  seedset: any = true;
  userData: any = {};
  gameTab: any = false;
  kenobetsdig: any = [
  ...
  rick2: any = [
    { hits: 0, percent: 55.76923 },
    { hits: 1, percent: 38.46154 },
    { hits: 2, percent: 5.769231 },
  ];
  rick3: any = [
    { hits: 0, percent: 41.093117 },
    { hits: 1, percent: 44.02834 },
    { hits: 2, percent: 13.663967 },
    { hits: 3, percent: 1.2145749 },
  ];
  rick4: any = [
    { hits: 0, percent: 29.98687 },
    { hits: 1, percent: 44.42499 },
    { hits: 2, percent: 21.419193 },
    { hits: 3, percent: 3.9391618 },
    { hits: 4, percent: 0.22978444 },
  ];
  rick5: any = [
    { hits: 0, percent: 21.657185 },
    { hits: 1, percent: 41.64843 },
    { hits: 2, percent: 27.76562 },
    { hits: 3, percent: 7.933034 },
    { hits: 4, percent: 0.9574352 },
    { hits: 5, percent: 0.03829741 },
  ];
  rick6: any = [
    { hits: 0, percent: 15.469417 },
    { hits: 1, percent: 37.126602 },
    { hits: 2, percent: 32.12879 },
    { hits: 3, percent: 12.692855 },
    { hits: 4, percent: 2.3799102 },
    { hits: 5, percent: 0.1969581 },
    { hits: 6, percent: 0.00547106 },
  ];
  rick7: any = [
    { hits: 0, percent: 10.919588 },
    { hits: 1, percent: 31.8488 },
    { hits: 2, percent: 34.3967 },
    { hits: 3, percent: 17.639334 },
    { hits: 4, percent: 4.5731606 },
    { hits: 5, percent: 0.5879778 },
    { hits: 6, percent: 0.03379183 },
    { hits: 7, percent: 0.00064365 },
  ];
  rick8: any = [
    { hits: 0, percent: 7.610622 },
    { hits: 1, percent: 26.471727 },
    { hits: 2, percent: 34.744144 },
    { hits: 3, percent: 22.236254 },
    { hits: 4, percent: 7.483354 },
    { hits: 5, percent: 1.3303741 },
    { hits: 6, percent: 0.1187834 },
    { hits: 7, percent: 0.00468112 },
    { hits: 8, percent: 0.00005851 },
  ];
  rick9: any = [
    { hits: 0, percent: 5.2323027 },
    { hits: 1, percent: 21.404875 },
    { hits: 2, percent: 33.50328 },
    { hits: 3, percent: 26.058107 },
    { hits: 4, percent: 10.944406 },
    { hits: 5, percent: 2.5256321 },
    { hits: 6, percent: 0.31180644 },
    { hits: 7, percent: 0.01909019 },
    { hits: 8, percent: 0.00049371 },
    { hits: 9, percent: 0.00000366 },
  ];
  rick10: any = [
    { hits: 0, percent: 3.5444632 },
    { hits: 1, percent: 16.878397 },
    { hits: 2, percent: 31.07159 },
    { hits: 3, percent: 28.820028 },
    { hits: 4, percent: 14.710222 },
    { hits: 5, percent: 4.236544 },
    { hits: 6, percent: 0.6789334 },
    { hits: 7, percent: 0.05747584 },
    { hits: 8, percent: 0.0023093 },
    { hits: 9, percent: 0.00003539 },
    { hits: 10, percent: 1.2e-9 },
  ];
  rickType1: any = {
    low: [0.7, 1.85],
    classic: [0, 3.96],
  ...
  constructor(
    ...
    this.httpService
      .postRequest('basic/Specificgame', { name: 'keno' }, this.token)
  ...
  showAllHis() {
    this.httpService
      .postRequest('keno/getAllBetHistory', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  over(data: any) {
    ...
    this.showpayout = parseFloat(this.rickType[this.risks][data.hits]).toFixed(
      2
    );
    this.showwinAmt = (
      parseFloat(this.rickType[this.risks][data.hits]) * 100
    ).toFixed(9);
  ...
  submitBet() {
    ...
    var data = {
      ...
      betNum: this.kenoNum,
      risklvl: this.rick,
      riskpayout: this.rickType[this.risks],
    };
  ...
  async sumbit_auto_bet() {
    ...
    for (var i = 0; i <= this.numberofbets; i++) {
      ...
      if (this.auto_bet_amount <= this.balance) {
        ...
        jQuery('.comrisk').removeClass('active');
        var data = {
          bet_amount: this.auto_bet_amount,
          currency: this.currency,
          type: this.risks,
          betNum: this.kenoNum,
          risklvl: this.rick,
  ...
  Details(id: any) {
    ...
    this.httpService
      .postRequest('keno/getDetails', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  userFav(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('keno/userFav', obj, this.token)
      .subscribe((resData: any) => {
  ...
  userLike(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('keno/userLiked', obj, this.token)
      .subscribe((resData: any) => {
  ...
  Seedsetting() {
    this.httpService
      .postRequest('keno/getHash', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  generateSeed() {
    ...
    this.httpService
      .postRequest('keno/generateHash', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  updateHash() {
    ...
    this.httpService
      .postRequest('keno/changeHash', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
