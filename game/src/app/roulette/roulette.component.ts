...
@Component({
  ...
  animations: [
    trigger('flipState', [
      ...
      state('0', style({ transform: 'rotate(-362deg)' })),
      state('1', style({ transform: 'rotate(-498.22deg)' })),
      state('2', style({ transform: 'rotate(-663.63deg)' })),
      state('3', style({ transform: 'rotate(-381.46deg)' })),
      state('4', style({ transform: 'rotate(-683.09deg)' })),
      state('5', style({ transform: 'rotate(-537.14deg)' })),
      state('6', style({ transform: 'rotate(-624.71deg)' })),
      state('7', style({ transform: 'rotate(-420.38deg)' })),
      state('8', style({ transform: 'rotate(-566.33deg)' })),
      state('9', style({ transform: 'rotate(-459.3deg)' })),
      state('10', style({ transform: 'rotate(-546.87deg)' })),
      state('11', style({ transform: 'rotate(-585.79deg)' })),
      state('12', style({ transform: 'rotate(-400.92deg)' })),
      state('13', style({ transform: 'rotate(-605.25deg)' })),
      state('14', style({ transform: 'rotate(-478.76deg)' })),
      state('15', style({ transform: 'rotate(-702.55deg)' })),
      state('16', style({ transform: 'rotate(-517.6800000000001deg)' })),
      state('17', style({ transform: 'rotate(-644.1700000000001deg)' })),
      state('18', style({ transform: 'rotate(-439.84000000000003deg)' })),
      state('19', style({ transform: 'rotate(-692.8199999999999deg)' })),
      state('20', style({ transform: 'rotate(-488.49deg)' })),
      state('21', style({ transform: 'rotate(-673.36deg)' })),
      state('22', style({ transform: 'rotate(-449.57deg)' })),
      state('23', style({ transform: 'rotate(-556.6deg)' })),
      state('24', style({ transform: 'rotate(-527.41deg)' })),
      state('25', style({ transform: 'rotate(-653.9000000000001deg)' })),
      state('26', style({ transform: 'rotate(-371.73deg)' })),
      state('27', style({ transform: 'rotate(-614.98deg)' })),
      state('28', style({ transform: 'rotate(-410.65deg)' })),
      state('29', style({ transform: 'rotate(-430.11deg)' })),
      state('30', style({ transform: 'rotate(-576.06deg)' })),
      state('31', style({ transform: 'rotate(-469.03deg)' })),
      state('32', style({ transform: 'rotate(-712.28deg)' })),
      state('33', style({ transform: 'rotate(-507.95000000000005deg)' })),
      state('34', style({ transform: 'rotate(-634.44deg)' })),
      state('35', style({ transform: 'rotate(-391.19deg)' })),
...
export class RouletteComponent implements OnInit {
  ...
  options2: Options = { floor: 0, ceil: 200 };
  value2: number = 100;
  betValarr: any = [];
  bet_amount: any = 0;
  head: any = true;
  ...
  wheelStyle: any = {};
  BallStyle: any = {};
  auto_bet_amount: any = 0;
  numberofbets: any = 0;
  stop_on_win: any = 0.0;
  stop_on_loss: any = 0.0;
  auto_bet: any = true;
  stop_init: any = 1;
  coinOne: any = 100;
  coinTwo: any = 1000;
  ...
  Det_gameResult: any;
  Det_seedstatus: any;
  withouthashserver: any;
  withouthashclient: any;
  ...
  //activebet
  betactive: any = false;
  autobetactive: any = false;
  activebet: any = false;
  autoactive: any = false;
  winnerpop: any = false;
  tiedpop: any = false;
  gameTab: any = false;
  resautoman: any = true;
  ...
  constructor(
    private httpService: ConnectionService,
    ...
    this.httpService
      .postRequest('basic/Specificgame', { name: 'roulette' }, this.token)
  ...
  getfavNlike() {
    ...
    if (favt.includes(game)) {
      this.fav_clr = 1;
    } else {
      this.fav_clr = 0;
    }
    if (liked.includes(game)) {
      this.like_clr = 1;
    } else {
      this.like_clr = 0;
    }
  ...
  favCount() {
    ...
    this.httpService
      .postRequest('basic/getFavCount', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  likeCount() {
    var obj = { name: 'roulette' };
    this.httpService
      .postRequest('basic/getLikeCount', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  getBalance(currency: any) {
    this.httpService
      .postRequest('basic/getBalance', { currency: this.currency }, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  cuurStatus(currency: any) {
    ...
    this.httpService
      .postRequest('basic/currStatus', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
          if (resData.msg.status == 0) {
  ...
  submitBet() {
    ...
    if (parseFloat(this.bet_amount) !== 0) {
      if (parseFloat(this.bet_amount) >= this.min_bet) {
        ...
        this.alert.error(
          'please Enter minimum of ' + this.min_bet + ' amount !'
        );
      }
    } else {
      this.activebet = false;
  ...
  async bettingInitiate(data: any) {
    ...
    if (auth == true) {
      ...
      var results: any = await this.httpService.postRequestUpdated(
        'roulette/getGameResult',
        data,
        this.token
      );
      if (results.success == 1) {
        ...
        setTimeout(() => {
          if (results.status == 'loser') {
            ...
            this.alert.success('Won the Game');
          }
          ...
          this.returnAmt =
            parseFloat(results.payOut) * parseFloat(results.bet_amount);
    ...
    if (parseFloat(this.bet_amount) !== 0) {
      ...
      for (var i = 0; i <= this.numberofbets; i++) {
        if (this.auto_bet_amount <= this.balance) {
          ...
          if (
            this.balance >= win_balance ||
            this.balance <= parseFloat(loss_balance) ||
            i == this.numberofbets ||
            this.stop_init == 0
          ) {
            this.auto_bet = true;
      ...
      if (this.bet_amount <= this.balance) {
        if (this.bet_amount <= 2000) {
          if (this.bet_amount <= 2000) {
            this.auto_bet_amount = this.bet_amount;
            if (type == 'half') {
              var obj = this.betValarr.find((o) => o.bet == 'half' + bet);
            } else if (type == 'row') {
              var obj = this.betValarr.find((o) => o.bet == 'row' + bet);
            } else if (type == 'segment') {
              var obj = this.betValarr.find((o) => o.bet == 'seg' + bet);
            } else {
              var obj = this.betValarr.find((o) => o.bet == bet);
            }
            if (obj == undefined) {
              this.betactive = true;
              this.autobetactive = true;
              ...
              if (type == 'half') {
                this.betcoins['half' + bet] = parseFloat(obj.amt);
              } else if (type == 'single') {
                this.betcoins['num' + bet] = parseFloat(obj.amt);
              } else if (type == 'row') {
                this.betcoins['row' + bet] = parseFloat(obj.amt);
              } else if (type == 'segment') {
                this.betcoins['seg' + bet] = parseFloat(obj.amt);
              } else if (type == 'type') {
                if (bet == 'even') {
                  this.betcoins['even'] = parseFloat(obj.amt);
                } else if (bet == 'red') {
                  this.betcoins['red'] = parseFloat(obj.amt);
                } else if (bet == 'black') {
                  this.betcoins['black'] = parseFloat(obj.amt);
                } else if (bet == 'odd') {
                  this.betcoins['odd'] = parseFloat(obj.amt);
                }
              }
            }
          } else {
  ...
  userFav(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('roulette/userFav', obj, this.token)
      .subscribe((resData: any) => {
  ...
  userLike(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('roulette/userLiked', obj, this.token)
      .subscribe((resData: any) => {
  ...
  Seedsetting() {
    this.httpService
      .postRequest('roulette/getHash', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  generateSeed() {
    var obj = {
      newserverSeed: this.newserverSeed,
      newclientSeed: this.newclientSeed,
      nonce: 0,
    };
    this.httpService
      .postRequest('roulette/generateHash', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  updateHash() {
    var obj = {
      ...
      newclientSeed: this.newclientSeed,
      nonce: 0,
      oldserverSeed: this.serverSeed,
    };
    this.httpService
      .postRequest('roulette/changeHash', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  allUser_history() {
    this.httpService
      .postRequest('roulette/getAllBetHistory', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  getDetails(id: any) {
    ...
    this.httpService
      .postRequest('roulette/getDetails', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  getgraph() {
    ...
    this.httpService
      .postRequest('basic/getgraphed', data, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
          if (resData.msg == 0) {
  ...
  getExacttime() {
    let data = { game: 'roulette' };
    this.httpService
      .postRequest('basic/getExacttime', data, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
          this.getgraph();
        }
