...
export class CoinflipComponent implements OnInit {
  ...
  Det_pro_amt: any;
  Det_series: any = 1;
  Det_result: any = [{ bet_result: 'head' }];
  Det_serverSeed: any;
  Det_clientSeed: any;
  Det_nounce: any;
  Det_status: any;
  ...
  constructor(
    ...
    this.httpService
      .postRequest('basic/Specificgame', { name: 'coinflip' }, this.token)
  ...
  gameId() {
    ...
    if (checkdata !== null && checkdata !== undefined && checkdata !== '') {
      ...
      this.httpService
        .postRequest('coinflip/getcurt', { _id: this.userId }, this.token)
        .subscribe((resData: any) => {
          if (resData) {
        ...
        setTimeout(() => {
          ...
          if (type == 'heads') {
            ...
            var storeData = {
              ...
              currency: this.currency,
              payout: this.payout,
              bet_result: this.bet_history,
              status: 'pending',
              amount: this.returnAmt,
            };
  ...
  submitBet(data: any) {
    ...
    this.httpService
      .postRequest('coinflip/saveBetHistory', data, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
          var hist = resData.history;
          this.bethistory = hist;
          this.payoutHistory = hist.slice(0, 12);
          if (resData.coinData.status == 'pending') {
            localStorage.setItem('gAmE-UsEr', resData.userId);
          } else {
  ...
  showAllHis() {
    this.httpService
      .postRequest('coinflip/getAllBetHistory', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  Details(id: any) {
    ...
    this.httpService
      .postRequest('coinflip/getDetails', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  updateHash() {
    ...
    this.httpService
      .postRequest('coinflip/changeHash', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  generateSeed() {
    ...
    this.httpService
      .postRequest('coinflip/generateHash', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  Seedsetting() {
    this.httpService
      .postRequest('coinflip/getHash', {}, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  userFav(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('coinflip/userFav', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  userLike(name: any) {
    var obj = { name: name };
    this.httpService
      .postRequest('coinflip/userLiked', obj, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
